# usage:
# R --slave --vanilla --file=RankingComparisionCLI_XMCDAv2.R --args "[inDirectory]" "[outDirectory]"
...
#inDirectory <- "/path/to/RankingComparision/tests/in1.v2"
#outDirectory <- "/path/to/RankingComparision/tests/out_tmp/"

# filenames

alternativesFile <- "alternatives.xml"
first_rankingFile <- "first_ranking.xml"
second_rankingFile <- "second_ranking.xml"
imageFile <- "image.xml"
...
loadXMCDAv3(xmcdaData, inDirectory, first_rankingFile, mandatory = TRUE, xmcdaMessages, "alternativesValues")
loadXMCDAv3(xmcdaData, inDirectory, second_rankingFile, mandatory = TRUE, xmcdaMessages, "alternativesValues")
...
if (is.null(results)){
  writeXMCDA(xmcdaMessages, paste(outDirectory,messagesFile, sep="/"))
  stop("Could not calculate RankingComparision.")
}
...
if (is.null(xResults)){
  writeXMCDA(xmcdaMessages, paste(outDirectory,messagesFile, sep="/"))
  stop("Could not convert RankingComparision results into XMCDA")
}
