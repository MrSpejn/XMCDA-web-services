# usage:
# R --slave --vanilla --file=OutrankingGraphCLI_XMCDAv2.R --args "[inDirectory]" "[outDirectory]"
...
#inDirectory <- "/path/to/OutrankingGraph/tests/in1.v2"
#outDirectory <- "/path/to/OutrankingGraph/tests/out_tmp/"

# filenames

alternativesFile <- "alternatives.xml"
outrankingFile <- "outranking.xml"
highlightedAlternativesFile <- "highlightedAlternatives.xml"
...
loadXMCDAv3(xmcdaData, inDirectory, outrankingFile, mandatory = TRUE, xmcdaMessages, "alternativesMatrix")
loadXMCDAv3(xmcdaData, inDirectory, highlightedAlternativesFile, mandatory = FALSE, xmcdaMessages, "alternativesValues")
loadXMCDAv3(xmcdaData, inDirectory, paramsFile, mandatory = TRUE, xmcdaMessages, "programParameters")
...
if (is.null(results)){
  writeXMCDA(xmcdaMessages, paste(outDirectory,messagesFile, sep="/"))
  stop("Could not calculate OutrankingGraph.")
}
...
if (is.null(xResults)){
  writeXMCDA(xmcdaMessages, paste(outDirectory,messagesFile, sep="/"))
  stop("Could not convert OutrankingGraph results into XMCDA")
}
