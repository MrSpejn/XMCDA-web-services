# usage:
# R --slave --vanilla --file=SelectedAlternativesCLI_XMCDAv2.R --args "[inDirectory]" "[outDirectory]"
...
#inDirectory <- "/path/to/SelectedAlternatives/tests/in1.v2"
#outDirectory <- "/path/to/SelectedAlternatives/tests/out_tmp/"

# filenames

alternativesFile <- "alternatives.xml"
selectionValuesFile <- "selection_values.xml"
seedFile <- "seed.xml"
imageFile <- "image.xml"
...
loadXMCDAv3(xmcdaData, inDirectory, selectionValuesFile, mandatory = TRUE, xmcdaMessages, "alternativesValues")
loadXMCDAv3(xmcdaData, inDirectory, seedFile, mandatory = TRUE, xmcdaMessages, "programParameters")
...
if (is.null(results)){
  writeXMCDA(xmcdaMessages, paste(outDirectory,messagesFile, sep="/"))
  stop("Could not calculate SelectedAlternatives.")
}
...
if (is.null(xResults)){
  writeXMCDA(xmcdaMessages, paste(outDirectory,messagesFile, sep="/"))
  stop("Could not convert SelectedAlternatives results into XMCDA")
}
