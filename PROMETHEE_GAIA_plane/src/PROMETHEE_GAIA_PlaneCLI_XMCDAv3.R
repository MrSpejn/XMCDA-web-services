# usage:
# R --slave --vanilla --file=PROMETHEE_GAIA_PlaneCLI_XMCDAv2.R --args "[inDirectory]" "[outDirectory]"
...
#inDirectory <- "/path/to/PROMETHEE_GAIA_Plane/tests/in1.v2"
#outDirectory <- "/path/to/PROMETHEE_GAIA_Plane/tests/out_tmp/"

# filenames

alternativesFile <- "alternatives.xml"
criteriaFile <- "criteria.xml"
promethee_profilesFile <- "promethee_profiles.xml"
criteria_weightsFile <- "criteria_weights.xml"
imageFile <- "image.xml"
...
loadXMCDAv3(xmcdaData, inDirectory, promethee_profilesFile, mandatory = TRUE, xmcdaMessages, "performanceTable")
loadXMCDAv3(xmcdaData, inDirectory, criteria_weightsFile, mandatory = TRUE, xmcdaMessages, "criteriaValues")
...
if (is.null(results)){
  writeXMCDA(xmcdaMessages, paste(outDirectory,messagesFile, sep="/"))
  stop("Could not calculate PROMETHEE_GAIA_Plane.")
}
...
if (is.null(xResults)){
  writeXMCDA(xmcdaMessages, paste(outDirectory,messagesFile, sep="/"))
  stop("Could not convert PROMETHEE_GAIA_Plane results into XMCDA")
}
